...
OUTPUT_JSON_PATH = '/shared_data/vs/output.json' 
...
def sensors_value_to_matrix(dict_sensors_t):
    ...
    for sensor in dict_sensors_t.keys():
        if not (np.isnan(dict_sensors_t[sensor]["value"]) or math.isnan(dict_sensors_t[sensor]["value"]) or pd.isna(dict_sensors_t[sensor]["value"])):
...
if __name__ == "__main__":
    ...
    predictions = {'coordinates': predictions[0].tolist(), 'predictions': predictions[1].tolist()}
